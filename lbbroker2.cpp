...
int amountOfRequestsPerClient = 50000;
...
client_thread(void *arg) {
    ...
    for (int request_nbr = 0; request_nbr != amountOfRequestsPerClient; request_nbr++) {
      //  Send request, get reply
      s_send(client, "HELLO" + std::to_string(request_nbr));
      std::string reply = s_recv(client);
      //std::cout << "Client" << (intptr_t)arg << " received: " << reply << std::endl;
    }
    return (NULL);
}
...
worker_thread(void *arg) {
    ...
    while (1) {
        ...
        // myfile << "Worker " << (intptr_t)arg << " received: " << request << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    int client_nbr;
    for (client_nbr = 0; client_nbr < amountOfClients; client_nbr++) {
        pthread_t client;
        pthread_create(&client, NULL, client_thread, (void *)(intptr_t)client_nbr);
    }
    int requests = amountOfRequestsPerClient;
    int client_nbr_requests = client_nbr * requests;
